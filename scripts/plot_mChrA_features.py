...
for chrom in gene_table[0].drop_duplicates():
    if chrom == "AG006_Contig10": # "contig_7_pilon_pilon" for Br62
...
plt.xticks(ticks=np.arange(0, end_posi/window_size, 20), labels=[round(n, 2) for n in np.arange(0, end_posi/(100*window_size), 0.2)], ha='right')
plt.savefig(f"{isolate_name}_mChrA.pdf")
plt.show()
